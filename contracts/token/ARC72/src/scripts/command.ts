...
vnsCmd
  ...
  .option("-d, --debug", "Debug the deployment", false)
  .action(async (options: VNSSetOwnerOptions) => {
...
export const getName: any = async (options: VNSResolverGetNameOptions) => {
  ...
  const node = namehash(options.node);
  const getNameR = await ci.name(
    options.nodeHash
      ? new Uint8Array(Buffer.from(options.nodeHash, "hex"))
      : namehash(options.node)
  );
  console.log("getNameR", getNameR);
  if (options.debug) {
...
interface registrarSetVersionOptions {
  apid: number;
...
export const registrarSetVersion: any = async (
  options: registrarSetVersionOptions
) => {
  const address = options.sender || addr;
  const secretKey = options.sk || sk;
  const ci = new CONTRACT(
  ...
  const registrarSetVersionR = await ci.set_version(
    BigInt(options.contractVersion),
    BigInt(options.deploymentVersion)
  );
  if (options.debug) {
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: registrarSetVersionOptions) => {
    const res = await registrarSetVersion(options);
    console.log(res);
  });

interface registrarApproveControllerOptions {
  apid: number;
  controller: string;
  approved?: boolean;
  debug?: boolean;
...
export const registrarApproveController: any = async (
  options: registrarApproveControllerOptions
) => {
  const address = options.sender || addr;
  const secretKey = options.sk || sk;
  const ci = new CONTRACT(
  ...
  const registrarApproveControllerR = await ci.approve_controller(
    options.controller,
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: registrarApproveControllerOptions) => {
    const res = await registrarApproveController(options);
    console.log(res);
  });

interface registrarIsControllerOptions {
  apid: number;
  controller: string;
  debug?: boolean;
}

export const registrarIsController: any = async (
  options: registrarIsControllerOptions
) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .action(async (options: registrarIsControllerOptions) => {
    const res = await registrarIsController(options);
    console.log(res);
  });

interface registrarSetGracePeriodOptions {
  apid: number;
  gracePeriod: number;
  debug?: boolean;
  simulate?: boolean;
...
export const registrarSetGracePeriod: any = async (
  options: registrarSetGracePeriodOptions
) => {
  const address = options.sender || addr;
  const secretKey = options.sk || sk;
  const ci = new CONTRACT(
  ...
  const registrarSetGracePeriodR = await ci.set_grace_period(
    BigInt(options.gracePeriod)
  );
  if (options.debug) {
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: registrarSetGracePeriodOptions) => {
    const res = await registrarSetGracePeriod(options);
    console.log(res);
  });

interface registrarMintOptions {
  apid: number;
  to: string;
  name: string;
  debug?: boolean;
...
export const registrarMint: any = async (options: registrarMintOptions) => {
  const address = options.sender || addr;
  const secretKey = options.sk || sk;
  const ci = new CONTRACT(
    Number(options.apid),
  ...
  const registrarMintR = await ci.mint(
    options.to,
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: registrarMintOptions) => {
...
interface VNSRegistrarGetRootNodeOptions {
  apid: number;
  debug?: boolean;
}

export const vnsRegistrarGetRootNode: any = async (
  options: VNSRegistrarGetRootNodeOptions
) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
...
registrarCmd
  ...
  .requiredOption("-a, --apid <number>", "The registrar ID")
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSRegistrarGetRootNodeOptions) => {
...
interface VNSRegistrarGetRootNodeNameOptions {
  apid: number;
  debug?: boolean;
}

export const vnsRegistrarGetRootNodeName: any = async (
  options: VNSRegistrarGetRootNodeNameOptions
) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
  ...
  return stripTrailingZeroBytes(
    Buffer.from(vnsRegistrarGetRootNodeNameR.returnValue).toString("utf-8")
  );
};
registrarCmd
  ...
  .requiredOption("-a, --apid <number>", "The registrar ID")
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSRegistrarGetRootNodeNameOptions) => {
...
export const vnsRegistrarPostUpdate: any = async (
  ...
  const vnsRegistrarPostUpdateR = await ci
    .post_update
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSExpirationOptions) => {
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: VNSReclaimOptions) => {
    const res = await reclaim(options);
    console.log(res);
  });

interface VNSRegistrarGetAppIdOptions {
  apid: number;
  name: string;
  debug?: boolean;
}

export const getAppId: any = async (options: VNSRegistrarGetAppIdOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
...
registrarCmd
  ...
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSRegistrarGetAppIdOptions) => {
    const res = await getAppId(options);
    console.log(res);
  });

interface VNSRegistrarUpdateSubnameRegistrarOptions {
  apid: number;
  resolver: number;
  node: string;
  debug?: boolean;
...
registrarCmd
  .command("update-subname-registrar")
  .requiredOption("-a, --apid <number>", "The VNS registrar ID")
  .requiredOption("-r, --resolver <number>", "The VNS resolver ID")
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSRegistrarUpdateSubnameRegistrarOptions) => {
    if (options.debug) {
      console.log("options", options);
    }
    try {
      const rootNode = await vnsRegistrarGetRootNode({
        apid: options.apid,
      });
      if (!rootNode) {
        throw new Error("Failed to get root node or empty");
      }
      const nodeHash = Buffer.from(rootNode).toString("hex");
      const nameR = await getName({
        apid: options.resolver,
        nodeHash,
      });
      const name = stripTrailingZeroBytes(Buffer.from(nameR).toString("utf8"));
      if (!nameR) {
        throw new Error("Failed to get name or empty");
      }
      console.log("name", name);
      const subnameRegistrar = {
        subnameRegistrar: name,
        contract: options.apid,
      };
      console.log("subnameRegistrar", subnameRegistrar);
      vnsRegistrarSetText({
        apid: options.apid,
        key: "subname_registrar",
        value: JSON.stringify(subnameRegistrar),
      });
    } catch (e) {
      console.log(e);
    }
...
interface VNSRegistrarGetOwnerOptions {
  apid: number;
  debug?: boolean;
}

export const getOwner: any = async (options: VNSRegistrarGetOwnerOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
  ...
  const owner = await ci.get_owner();
...
registrarCmd
  ...
  .requiredOption("-a, --apid <number>", "The VNS registrar ID")
  .option("-d, --debug", "Debug mode")
  .action(async (options: VNSRegistrarGetOwnerOptions) => {
    const res = await getOwner(options);
    console.log(res);
  });
...
export const getVersion: any = async (options: GetVersionOptions) => {
  const contract = new VNSRegistrarClient(
    {
      resolveBy: "id",
      id: Number(options.apid),
      sender: {
        addr: addr,
        sk: sk,
      },
  ...
  return `Contract Version: ${contractVersion} Deployment Version: ${deploymentVersion}`;
};
...
registrarCmd
  ...
  .requiredOption("-a, --apid <number>", "The VNS registrar ID")
  .option("-d, --debug", "Debug mode")
  .action(async (options: GetVersionOptions) => {
    const res = await getVersion(options);
    console.log(res);
  });

interface VNSRegistrarSetTextOptions {
  apid: number;
  key: string;
  value: string;
  debug?: boolean;
...
export const vnsRegistrarSetText: any = async (
  options: VNSRegistrarSetTextOptions
) => {
  const address = options.sender || addr;
  const secretKey = options.sk || sk;
  const ci = new CONTRACT(
...
registrarCmd
  .command("set-text")
  .requiredOption("-a, --apid <number>", "The VNS registrar ID")
  .requiredOption("-k, --key <string>", "The key to set")
  .requiredOption("-v, --value <string>", "The value to set")
  .option("-d, --debug", "Debug mode")
  .option("-s, --simulate", "Simulate the transaction")
  .action(async (options: VNSRegistrarSetTextOptions) => {
    const res = await vnsRegistrarSetText(options);
    console.log(res);
  });
...
interface ARC72TokenURIOptions {
  apid: number;
  name: string;
  debug?: boolean;
}

export const arc72TokenURI: any = async (options: ARC72TokenURIOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
...
arc72Cmd
  ...
  .option("-d, --debug", "Debug mode")
  .action(async (options: ARC72TokenURIOptions) => {
    console.log({ options });
